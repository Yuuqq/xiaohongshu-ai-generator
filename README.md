...
- **顶部**：5步进度指示器（当前步骤高亮粉色）
- **左侧/上方**：内容输入区（大文本框 + 实时字数统计 + 清空按钮）
- **右侧**：实时预览区（Canvas 渲染，可实时看到卡片效果）
- **中部**：风格与模板选择区（卡片式选择，支持多种主题）
- **底部**：生成控制 + 导出按钮区

**整体风格**：粉色系主色调（#FF2442 小红书经典色），轻盈现代，响应式设计，移动端也友好。

> **提示**：实际界面为单页应用，操作流畅，预览实时更新。

---

## 🖼️ 典型生成效果示例

工具支持多种视觉风格，以下是常见主题的**预期效果描述**（实际生成效果更精美）：

### 科技 / AI 主题
- **色调**：蓝紫渐变 + 科技线条/节点装饰
- **适合内容**：AI、科技、效率工具、未来趋势
- **视觉特点**：现代、简洁、专业，带轻微几何或电路元素

### 好物 / 种草主题
- **色调**：粉色、柔和暖色、甜美风格
- **适合内容**：美妆、护肤、穿搭、好物推荐
- **视觉特点**：可爱、温馨、吸引眼球，装饰元素丰富

### 教程 / 步骤主题
- **色调**：清新绿、橙色或简约黑白
- **适合内容**：教程、干货、步骤指南、知识分享
- **视觉特点**：清晰编号、结构化布局、易读性强

> **自定义能力**：支持调整装饰强度、纹理、颜色倾向，可生成同内容的不同变体，方便 A/B 测试。

**【图片展示优化建议】**  
当前版本为文字描述。如需添加真实截图，请按下方指南操作（非常简单）。
...
为了让 README 更加图文并茂，建议添加真实界面截图：

1. 打开在线 Demo：https://yuuqq.github.io/xiaohongshu-ai-generator/
2. 截图以下内容（推荐使用浏览器开发者工具或截图工具）：
   - 主界面总览（显示 5 步流程）
   - 不同模板生成的卡片效果（科技风、好物风、教程风）
   - 设置面板（API Key 输入）
3. 在仓库根目录创建 `screenshots/` 文件夹
4. 将截图命名为：
   - `main-ui.png`
   - `tech-example.png`
   - `lifestyle-example.png`
   - `tutorial-example.png`
5. 在 README 中替换对应位置为相对路径，例如：
   ```markdown
   ![主界面总览](screenshots/main-ui.png)
   ```

这样图片就能稳定显示，且加载快（GitHub 原生支持）。
